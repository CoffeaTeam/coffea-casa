...
def test_job_extra_directives_contains_required_fields(mock_environment):
    ...
    with patch("coffea_casa.coffea_casa.security_obj") as mock_sec_obj, \
        ...
        # ("/bin/sh", generated by dask_jobqueue) as a file to transfer from
        # the submit host and exec as "./sh" instead of using the image's
        # own shell - every real docker-universe job fails immediately.
        assert directives.get("transfer_executable") is False
