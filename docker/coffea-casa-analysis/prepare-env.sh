...
if [ ! -z "$_CONDOR_JOB_AD" ]; then
    ...
    if [ "$TLS_ENV" == "true" ]; then
        ...
        echo $HTCONDOR_COMAND --contact-address tls://$HOST:$PORT --listen-address tls://0.0.0.0:8787 1>&2
        exec $HTCONDOR_COMAND --contact-address tls://$HOST:$PORT --listen-address tls://0.0.0.0:8787
